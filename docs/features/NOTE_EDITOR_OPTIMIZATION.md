...
### 近期已实现（2026-03，Task 7.5.2～7.6.2）
- ✅ **标题下元信息行**：弱化展示更新时间、标签，小字号不抢正文
- ✅ **长文目录/大纲**：更多菜单中「目录」入口，H1/H2/H3 跳转
- ✅ **聚焦态/选区态**：去掉粗边框，用 `focus-within` 微弱底色/左边框表达可编辑
- ✅ **Placeholder 与首次轻提示**：友好引导语，移动端首次进入编辑态时 Toast 提示（仅一次）

### 近期已实现补充（2026-03-18）
- ✅ **粘贴 Markdown 排版修复**：多行/疑似 Markdown 优先走 `text/plain` 保留换行；修复 HTML 转换吞换行导致“缩成一团”
- ✅ **Dock 全选/取消全选**：多选模式 Dock 新增「全选」入口，可一键选中文件夹 + 笔记，再次点击可取消
- ✅ **空白区域取消多选**：列表页/根目录页在多选模式下，点击卡片外空白即可退出多选（document 捕获级兜底）
- ✅ **移动端完成按钮去重**：仅保留底部悬浮「完成」，取消右上角重复入口
- ✅ **移动端视口与键盘遮挡修复**：增强 `visualViewport` 变量刷新与编辑页全屏高度保护；聚焦时自动滚动避免被键盘遮挡
...
#### 4.1 智能补全
- **双向链接补全**：输入 `[[` 时自动补全笔记标题（候选列表、↑↓/Enter 导航），插入 `[[noteId|显示名称]]`（当前按“内容末尾”近似光标位置）
- **标签补全**：输入 `#` 时自动补全已有标签（候选列表、↑↓/Enter 导航），插入 `#tag `（避免与 Markdown 标题 `# ` 冲突）
- **Markdown 语法补全**：输入 `#` 时自动补全标题语法

> 近期落地：补全逻辑已从容器组件中抽离为 `useLinkComplete` / `useTagComplete`，补全 UI 统一由 `NoteEditor` 渲染，便于复用与测试。
