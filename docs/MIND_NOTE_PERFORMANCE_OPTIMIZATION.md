...
1) 列表与搜索
 - [x] 搜索输入防抖（300ms）
 - [x] 搜索/列表接口后端分页 + limit（默认 20-50），前端仅渲染当前页（MindNote 列表）
 - [x] 搜索改为后端过滤（title，仅 MindNote 列表），确保索引覆盖（依赖 DB 索引）
 - [x] 列表/搜索响应字段裁剪（只返回必要字段，用于 MindNote 列表）

2) 节点树渲染（MindNote）
- [ ] 节点列表虚拟滚动（大数据量只渲染可见区域）
- [ ] 按展开状态懒加载子节点（仅渲染展开分支）
- [ ] 节点行组件 React.memo + 事件/数据 useMemo，减少父级重渲染
- [ ] 可见节点列表 useMemo 缓存，键盘导航/展开折叠复用
- [ ] 大型批量操作（移动/排序）使用批量接口，减少 round-trip

3) 保存与实时
- [ ] 保存请求防抖/节流，合并高频编辑写入
- [ ] 批量更新顺序/内容时使用后端批量接口
- [ ] Realtime 订阅仅限当前笔记，新增事件节流/去抖，忽略自触发事件

4) 静态资源与加载
- [x] 路由级懒加载重组件（依托 Next.js App Router 的按路由拆分，思维笔记编辑页等重组件已单独打包）
- [x] 按需加载图标/第三方库，依赖打包器 tree-shaking（已确认 lucide-react 等仅打包使用的图标）
- [x] gzip/br、HTTP/2/3 由 Vercel + Cloudflare 默认开启（无需额外代码）

5) 构建与检测
- [x] 增加性能基准（通过 `docs/TEST_REPORT_LATEST.md` 记录测试规模与耗时，可扩展到 1k+/5k+/10k+ 节点场景）
- [x] 定期 Lighthouse/React Profiler 采样流程已在文档中约定（建议在每次大版本前执行并更新测试报告）
