...
#### 11.1 移动端编辑页全屏高度稳定性
- **现象**：从预览页进入编辑页，偶发编辑层只显示约 1/3；编辑/完成切换时复现。
- **落地**：
  - `ViewportVars` 增加 `pageshow/focus/visibilitychange` 等事件刷新 `--vvh/--vv-offset-top/--vv-bottom-inset`，并增加首帧兜底刷新，降低路由切换时变量卡住概率。
  - 编辑页全屏容器对 `--vv-offset-top` 做保护：仅允许负 offset 生效，避免异常正 offset 下推导致可视区域变小。

#### 11.2 键盘遮挡输入位置
- **现象**：编辑时正在输入的段落被键盘遮挡。
- **落地**：移动端写作模式下，textarea 聚焦时自动 `scrollIntoView({ block: "center" })`，优先保证光标附近可见。

#### 11.3 写作模式入口收敛
- **现象**：移动端写作模式右上与右下同时出现“完成”按钮，入口重复。
- **落地**：取消右上角“完成”，仅保留底部悬浮“完成”。

#### 11.4 粘贴 Markdown 排版
- **现象**：粘贴 Markdown 后换行/段落被合并，用户需要逐行手动排版。
- **落地**：粘贴策略优先信任 `text/plain`（多行或疑似 Markdown），并修复 HTML 转换逻辑对换行的处理，避免“压成一团”。
...
## 10. 移动端「六模式」体验对齐（参考主流产品）

以下六项关键模式来自对 iOS 备忘录、飞书、语雀、Notion 等产品的归纳，用于在需求层面对齐「单画布、顶栏、键盘工具栏、块级交互、阅读体验、状态表达」等体验目标。具体可执行任务已拆解至 **`docs/tasks/NOTE_ENHANCEMENT_TASKS.md`** 的 **Phase 7**，本 PRD 与之配套使用。

| 模式 | 说明 | 对应 Task |
|------|------|-----------|
| 1) 单画布 | 标题即画布首行，无两个框；字号/字重/间距区分；默认阅读态点击进入编辑 | Task 7.1.1 ~ 7.1.3 |
| 2) 顶部 App Bar | 左返回、中状态、右更多；完成/退出编辑入口 | Task 7.2.1 ~ 7.2.3 |
| 3) 键盘上方工具栏 | 常驻格式化+插入；选中态切换；插入块 `/` 或 `+` | Task 7.3.1 ~ 7.3.3 |
| 4) 块/段落级交互 | 段落与块级样式；图片/附件独立块与 caption | Task 7.4.1 ~ 7.4.2 |
| 5) 阅读体验优先 | 排版系统、元信息行、长文目录/大纲 | Task 7.5.1 ~ 7.5.3 |
| 6) 减少边框用状态表达 | 聚焦/选区态样式；placeholder 与首次轻提示 | Task 7.6.1 ~ 7.6.2 |

- **任务文档**：`docs/tasks/NOTE_ENHANCEMENT_TASKS.md`（Phase 7）
- **思路与索引**：`docs/features/优化思路.md`
