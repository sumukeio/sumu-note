...
- **搜索功能**：
  ...
  - 搜索高亮：自动高亮所有匹配的搜索词，支持快捷键导航（`Ctrl+G` / `Ctrl+Shift+G`）
- **回收站**：删除的笔记可恢复，支持彻底删除
- **暗色模式**：支持亮色/暗色主题切换
...
- **匹配项导航**：支持 `Ctrl+G` / `Cmd+G`（下一个）、`Ctrl+Shift+G` / `Cmd+Shift+G`（上一个）快捷键导航
- **可视化导航**：提供导航按钮，显示当前匹配项位置（如 "2/5"）
...
- **全局全文搜索**
  ...
  - 支持快捷键导航：`Ctrl+G` / `Cmd+G`（下一个）、`Ctrl+Shift+G` / `Cmd+Shift+G`（上一个）
  - 提供可视化导航按钮，显示当前匹配项位置（如 "2/5"）
