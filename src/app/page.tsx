...
function LandingPageInner() {
  ...
  // issue002：dashboard 鉴权失败回流时打开登录并提示
  useEffect(() => {
    if (searchParams.get("auth") !== "required") return;
    toast({
      title: "需要重新登录",
      description:
        "未检测到有效登录状态，请再试一次。若持续失败，请关闭无痕模式。",
      variant: "default",
      duration: 5000,
    });
    openAuth("login");
    ...
    <div className="min-h-screen bg-black text-white flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center justify-center text-center px-4 mt-20 relative">
        ...
        <p className="text-lg text-zinc-400 max-w-2xl mb-10 leading-relaxed z-10">
          极简设计，AI 驱动。支持长按多选、智能 Dock 交互，以及 AWS
          企业级数据存储。
          <br />
          专为独立开发者和高效能人士打造。
        </p>
        ...
        <div className="mt-24 grid grid-cols-1 md:grid-cols-3 gap-8 text-left max-w-4xl w-full border-t border-zinc-800 pt-12 z-10">
          <FeatureItem
            title="极速体验"
            desc="基于 Next.js App Router 构建，快如闪电。"
          />
          <FeatureItem
            title="数据安全"
            desc="Supabase + AWS S3 双重保障，数据归你所有。"
          />
          <FeatureItem
            title="AI 赋能"
            desc="内置 Gemini 3 Pro 模型，自动整理与续写。"
          />
        </div>
      </main>
